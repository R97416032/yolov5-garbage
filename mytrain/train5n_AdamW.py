...
python train.py --data garbage.yaml --weights pretrained/yolov5n.pt --img 640
Usage:
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        if resume:
            assert start_epoch > 1, f'{weights} training to {epochs} epochs is finished, nothing to resume.'
    ...
    for epoch in range(start_epoch, epochs+1):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ni = i + nb * (epoch-1)  # number integrated batches (since train start)
            ...
            if RANK in [-1, 0]:
                ...
                pbar.set_description(('%10s' * 2 + '%10.4g' * 5) % (
                    f'{epoch}/{epochs}', mem, *mloss, targets.shape[0], imgs.shape[-1]))
        ...
        if RANK in [-1, 0]:
            ...
            final_epoch = (epoch  == epochs) or stopper.possible_stop
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default=ROOT / 'myweights/R_yolov5n.pt', help='initial weights path')
    ...
    parser.add_argument('--name', default='5n_AdamW', help='save to project/name')
